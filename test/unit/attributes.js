...
test( "non-lowercase boolean attribute getters should not crash (gh-3133)", function() {
	expect( 6 );
	...
	jQuery.each({
		...
		try {
			strictEqual( jQuery.find.attr( elem[ 0 ], original, false ), lowercased,
				"jQuery.find.attr( '" + original + "' ) should return the lowercased name" );
		} catch ( e2 ) {
			ok( false, "jQuery.find.attr( '" + original + "' ) threw" );
		}
	});
});
